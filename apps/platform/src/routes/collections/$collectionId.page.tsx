...
const ChallengeListRow = memo(ChallengeListRowInner);

function ChallengeListRowInner({
  challenge,
  index,
  isPassed,
  onClick,
...
export function CollectionPage() {
  ...
  const childCollections = useMemo(() => {
    ...
    return collection.childCollectionIds
      .map((childCollectionId) =>
        collections.find((currentCollection) => currentCollection.id === childCollectionId),
      )
      .filter((childCollection): childCollection is ChallengeCollection =>
        Boolean(childCollection),
      );
  }, [collection]);
  const childCollectionGroups = useMemo(
    () => groupChildCollections(childCollections),
    [childCollections],
  );
  const challenges = useMemo(() => {
    if (!collection) return [];
    return collection.challengeIds
