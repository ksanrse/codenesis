...
This folder contains the Svelte UI for Codenesis. Also follow the repository root `AGENTS.md`.
...
- Shared domain rules must come from packages, not from route-local constants. For challenge ranks, MMR, and level labels, import from `@codenesis/challenges`.
...
- `ChallengeLayout.svelte` owns the description, Monaco editor, and output panes.
- Editor layout styles are scoped through `.challenge-layout` and the shared global stylesheet.
- `EditorPanel.svelte` owns Monaco models and saves changed source through `local-db.ts`.
